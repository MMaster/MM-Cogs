...
from mmaiuser.abc import CompositeMetaClass
from mmaiuser.common.cache import Cache
from mmaiuser.common.constants import (
...
from mmaiuser.common.enums import ScanImageMode
from mmaiuser.common.utilities import is_embed_valid, is_using_openai_endpoint
from mmaiuser.dashboard_integration import DashboardIntegration
from mmaiuser.messages_list.entry import MessageEntry
from mmaiuser.random_message_task import RandomMessageTask
from mmaiuser.response.response_handler import ResponseHandler
from mmaiuser.settings.base import Settings
...
class MMAIUser(
    ...
    def __init__(self, bot):
        super().__init__()
        self.bot: Red = bot
        self.config = Config.get_conf(self, identifier=141445739606410539)
        ...
        default_guild = {
            ...
            "function_calling_default_location": [52.52000, 13.40495], # Berlin
    ...
    async def on_message_without_command(self, message: discord.Message):
        ...
        if rate_limit_reset > datetime.now():
            ...
            if (
                ...
                await ctx.react_quietly("💤", message="`mmaiuser` is ratedlimited")
            return
    ...
    async def initialize_openai_client(self, ctx: commands.Context = None):
        ...
        if base_url and str(base_url).startswith(OPENROUTER_URL):
            ...
            headers = {
                "HTTP-Referer": "https://mmaiuser.zhao.gg",
                "X-Title": "mmaiuser",
            }
        else:
        ...
        if not api_key and (not base_url or api_type == "openrouter"):
            if ctx:
                error_message = (
                    f"{api_type} API key not set for `mmaiuser`. "
                    f"Please set it with `{ctx.clean_prefix}set api {api_type} api_key,API_KEY`"
                )
                return await ctx.send(error_message)
            else:
                return logger.error(
                    f'{api_type} API key not set for "mmaiuser" yet! Please set it with: [p]set api {api_type} api_key,API_KEY'
                )

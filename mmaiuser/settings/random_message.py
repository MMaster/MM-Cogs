...
from mmaiuser.common.constants import DEFAULT_RANDOM_PROMPTS
from mmaiuser.settings.utilities import get_tokens
...
class RandomMessageSettings(MixinMeta):
    ...
    async def randommessage(self, _):
        """ Configure the random message event
            **(Must be enabled by bot owner first using `[p]mmaiuser random toggle`)**

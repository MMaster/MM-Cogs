...
from mmaiuser.common.utilities import is_using_openrouter_endpoint
from mmaiuser.settings.utilities import get_tokens, truncate_prompt

logger = logging.getLogger("red.mmaster.mmaiuser")


class OwnerSettings(MixinMeta):
    @commands.group(aliases=["mm_ai_userowner"])
    @checks.is_owner()
    async def mmaiuserowner(self, _):
        """For some settings that apply bot-wide."""
        pass

    @mmaiuserowner.command(name="maxpromptlength")
    ...
    @mmaiuserowner.command(name="maxtopiclength")
    ...
    async def endpoint(self, ctx: commands.Context, url: Optional[str]):
        ...
        Reset to official OpenAI endpoint with `[p]mmaiuseradmin endpoint clear`
        """
    ...
    @mmaiuserowner.command(name="exportconfig")
    async def export_config(self, ctx: commands.Context):
        ...
        await ctx.send(
            file=discord.File(path, filename="mmaiuser_config.json")
        )
        await ctx.tick()

    @mmaiuserowner.command(name="importconfig")
    ...
    @mmaiuserowner.command(name="prompt")
    async def global_prompt(self, ctx: commands.Context, *, prompt: Optional[str]):
        """ Set the global default prompt for mmaiuser.

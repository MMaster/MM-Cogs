...
from mmaiuser.common.constants import FUNCTION_CALLING_SUPPORTED_MODELS
from mmaiuser.common.enums import MentionType
from mmaiuser.common.utilities import (get_enabled_tools,
                                     is_using_openai_endpoint,
                                     is_using_openrouter_endpoint)
from mmaiuser.settings.functions import FunctionCallingSettings
from mmaiuser.settings.history import HistorySettings
from mmaiuser.settings.image_request import ImageRequestSettings
from mmaiuser.settings.image_scan import ImageScanSettings
from mmaiuser.settings.owner import OwnerSettings
from mmaiuser.settings.prompt import PromptSettings
from mmaiuser.settings.random_message import RandomMessageSettings
from mmaiuser.settings.response import ResponseSettings
from mmaiuser.settings.triggers import TriggerSettings
from mmaiuser.settings.utilities import get_config_attribute, get_mention_type
...
class Settings(
    ...
    async def forget(self, ctx: commands.Context):
        ...
        See `[p]mmaiuser triggers public_forget` to allow non-admins to use this command.
        """
        if (
    ...
    @mmaiuser.command(aliases=["settings", "showsettings"])
    ...
    async def model(self, ctx: commands.Context, model: str):
        """Changes chat completion model
         ...
         To see a list of available models, use `[p]mmaiuser model list`
         (Setting is per server)
        ...
        **Arguments**
        ...
        if await self.config.guild(ctx.guild).function_calling() and model not in FUNCTION_CALLING_SUPPORTED_MODELS:
            return await ctx.send(":warning: Can not select model that with no build-in support for function calling!\nSwitch function calling off using `[p]mmaiuser functions toggle` or select a model that supports function calling.")
    ...
    @mmaiuser.command(name="optinbydefault", alias=["optindefault"])
    @checks.admin_or_permissions(manage_guild=True)
